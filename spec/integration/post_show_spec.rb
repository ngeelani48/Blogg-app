...
RSpec.feature "Post Show Page", type: :feature do
  ...
  let!(:post) { Post.create(author: user, title: 'First Post', text: 'This is my first post.', comments_counter: 0, likes_counter: 0) }
  let!(:comment1) { Comment.create(author: user, post: post, text: 'I like it') }
  let!(:comment2) { Comment.create(author: user, post: post, text: 'Nice post') }
  ...
  scenario "User can add a comment to the post" do
    ...
    expect(page).to have_content("#{user.name}: This is a new comment") # Update the expectation to use the user variable
  end
